...
function Controls (props) {
  ...
  const isUsersFileLoaded = useSelector(state => Boolean(Object.keys(state.characters.characters).length))
  const isTranscriptFileLoaded = useSelector(state => Boolean(state.script.transcript))
  ...
  return (
    <div className="controls">
      ...
      <div className="properties">
        <Input label="Title" value={title} onChange={(e) => dispatch(setTitle(e))} />
        <Input label="Author" value={author} onChange={(e) => dispatch(setAuthor(e))} />
      </div>
      <button className="button" onClick={handleClickOpenFileButton}>Add a file</button>
